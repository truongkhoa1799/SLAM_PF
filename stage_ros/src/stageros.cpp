...
// libstage
#include </home/khoa1799/catkin_ws/src/SLAM/stage/Stage/libstage/stage.hh>
...
class StageNode
{
  private:
    ...
    stage_ros::fiducials * fiducialsMsgs;
    ...
    // random generators for odometry error
...
StageNode::StageNode(int argc, char** argv, bool gui, const char* fname, bool use_model_names)
{
  ...
  this->fiducialsMsgs = new stage_ros::fiducials[numRobots];
...
StageNode::SubscribeModels()
{
  ...
  for (size_t r = 0; r < this->positionmodels.size(); r++)
  {
    ...
    fiducials_pubs_.push_back(n_.advertise<stage_ros::fiducials>(mapName(FIDUCIALS,r,static_cast<Stg::Model*>(positionmodels[r])), 10));
...
StageNode::WorldCallback()
{
  ...
  for (size_t r = 0; r < this->fiducialmodels.size(); r++)
		{			
			...
			if (fiducials.size() > 0)
			{
		      		this->fiducialsMsgs[r].header.frame_id = mapName("base_laser_link", r, static_cast<Stg::Model*>(positionmodels[r]));
  ...
  for (size_t r = 0; r < this->lasermodels.size(); r++)
    ...
    //static std::vector<nav_msgs::Odometry> odom;
    //odom.resize(this->positionmodels.size());
    for (size_t r = 0; r < this->positionmodels.size(); r++)
      {
    ...
    if(first) {
      Stg::Velocity error = this->positionmodels[r]->GetIntegrationError();
      sigma_x = error.x;
      sigma_y = error.y;
      sigma_z = error.z;
      sigma_a = error.a;
    ...
    // set random integration error
    boost::normal_distribution<> ndx(0., sigma_x);
    boost::normal_distribution<> ndy(0., sigma_y);
    boost::normal_distribution<> ndz(0., sigma_z);
    boost::normal_distribution<> nda(0., sigma_a);
    boost::variate_generator<boost::mt19937 &, boost::normal_distribution<> > x_dist(rng, ndx);
    boost::variate_generator<boost::mt19937 &, boost::normal_distribution<> > y_dist(rng, ndy);
    boost::variate_generator<boost::mt19937 &, boost::normal_distribution<> > z_dist(rng, ndz);
    boost::variate_generator<boost::mt19937 &, boost::normal_distribution<> > a_dist(rng, nda);
    this->positionmodels[r]->SetIntegrationError(Stg::Velocity(x_dist(), y_dist(), z_dist(), a_dist()));
    ...
    /*
    if(first)
      odom[r] = this->odomMsgs[r];
    else
    {
      // get difference
      double dx = this->odomMsgs[r].pose.pose.position.x - old_odom.pose.pose.position.x;
      double dy = this->odomMsgs[r].pose.pose.position.y - old_odom.pose.pose.position.y;
      double theta_old = tf::getYaw(old_odom.pose.pose.orientation);
      double theta_new = tf::getYaw(this->odomMsgs[r].pose.pose.orientation);
      double dtheta = theta_new - theta_old;

      double vel_error = x_dist();
      double a_error = a_dist();

      // apply length error + rotation error
      dx *= vel_error;
      dy *= vel_error;

      double tmp = dtheta * (a_error - 1.) / 2.;
      double x = dx * std::cos(tmp) - dy * std::sin(tmp);
      double y = dy * std::cos(tmp) + dx * std::sin(tmp);
      double t = tf::getYaw(odom[r].pose.pose.orientation) + dtheta * a_error;

      odom[r].pose.pose.position.x = odom[r].pose.pose.position.x + x;
      odom[r].pose.pose.position.y = odom[r].pose.pose.position.y + y;
      odom[r].pose.pose.orientation = tf::createQuaternionMsgFromYaw(t);
      odom[r].twist.twist.linear.x = this->odomMsgs[r].twist.twist.linear.x * vel_error;
      odom[r].twist.twist.linear.y = this->odomMsgs[r].twist.twist.linear.y;
      odom[r].twist.twist.angular.z = this->odomMsgs[r].twist.twist.linear.z * a_error;
      odom[r].header.frame_id = mapName("odom", r,static_cast<Stg::Model*>(positionmodels[r]));
      odom[r].header.stamp = sim_time;

      this->odom_pubs_[r].publish(odom[r]);

      // broadcast odometry transform
      tf::Quaternion odomQ;
      tf::quaternionMsgToTF(odom[r].pose.pose.orientation, odomQ);
      tf::Transform txOdom(odomQ,
                           tf::Point(odom[r].pose.pose.position.x,
                                     odom[r].pose.pose.position.y, 0.0));
      tf.sendTransform(tf::StampedTransform(txOdom, sim_time,
                                            mapName("odom", r,static_cast<Stg::Model*>(positionmodels[r])),
                                            mapName("base_footprint", r,static_cast<Stg::Model*>(positionmodels[r]))));
    }
    nav_msgs::Odometry old_odom = this->odomMsgs[r];
    */
